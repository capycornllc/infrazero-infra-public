...
if [ -f "${SCRIPT_DIR}/db-patroni.sh" ]; then
  ...
  echo "[db-replica] missing shared db-patroni.sh" >&2
  exit 1
fi

infrazero_load_env_files \
  /etc/infrazero/db-replica.env \
...
apt_get() {
  ...
  INFRAZERO_APT_CLEAN_LISTS="${INFRAZERO_APT_CLEAN_LISTS:-false}" \
    infrazero_apt_get "db-replica" "$@"
}

infrazero_require_env "DB_TYPE" "db-replica"
infrazero_require_env "DB_VERSION" "db-replica"
infrazero_require_env "DB_PRIMARY_HOST" "db-replica"
infrazero_require_env "DB_REPLICATION_USER" "db-replica"
infrazero_require_env "DB_REPLICATION_PASSWORD" "db-replica"
...
setup_patroni() {
  ...
  infrazero_require_env "PATRONI_SCOPE" "db-replica"
  infrazero_require_env "PATRONI_ETCD_HOSTS" "db-replica"
