...
infrazero_require_env "K3S_TOKEN" "node1"
infrazero_require_env "EGRESS_LOKI_URL" "node1"
...
if ! command -v argocd >/dev/null 2>&1; then
  ...
  if infrazero_retry "node1" 5 2 curl -fsSL -o /usr/local/bin/argocd "$ARGOCD_CLI_URL"; then
...
infrazero_retry "node1" 10 5 curl -sfL https://get.k3s.io -o /tmp/k3s-install.sh
...
infrazero_retry "node1" 10 5 kubectl apply --server-side --force-conflicts -n argocd -f "$ARGOCD_INSTALL_MANIFEST_URL"
...
cert_check() {
  ...
  while [ "$elapsed" -lt "$max_wait" ]; do
    ...
    while IFS= read -r line; do
      ...
      if [ "$ready" = "True" ]; then
        cert_summary="${cert_summary}  [ready] ${domain} (${ns}/${name})\n"
      else
        all_ready=false
        cert_summary="${cert_summary}  [not-ready] ${domain} (${ns}/${name}): ${message}\n"
  ...
  while IFS= read -r line; do
    ...
    domain=$(echo "$line" | jq -r '.spec.dnsNames[0] // "unknown"')
    if [ "$ready" = "True" ]; then
      echo "[node1]   [ready] ${domain} (${ns}/${name})"
    else
      echo "[node1]   [not-ready] ${domain} (${ns}/${name}): ${message}"
    fi
  done < <(kubectl get certificates --all-namespaces -o json 2>/dev/null | jq -c '.items[]')
