...
apt_get() {
  ...
  INFRAZERO_APT_CLEAN_LISTS="${INFRAZERO_APT_CLEAN_LISTS:-false}" \
    infrazero_apt_get "pgbouncer" "$@"
}

infrazero_require_env "DB_PRIMARY_HOST" "pgbouncer"
infrazero_require_env "PGBOUNCER_AUTH_USER" "pgbouncer"
infrazero_require_env "PGBOUNCER_AUTH_PASSWORD" "pgbouncer"
...
resolve_private_ip() {
  infrazero_private_ip_for_cidr "${PRIVATE_CIDR:-}"
}
