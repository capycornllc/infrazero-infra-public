#!/usr/bin/env bash
# Shared db bootstrap for all providers. Provider wrappers
# (bootstrap/<provider>/db.sh) exec this script; cloud specifics (volume
# discovery, attach-wait timings) come from the provider adapter
# (see docs/provider-adapter-contract.md).
set -euo pipefail
...
if [ -f "${SCRIPT_DIR}/common-base.sh" ]; then
  # shellcheck disable=SC1091
  source "${SCRIPT_DIR}/common-base.sh"
elif [ -f "${SCRIPT_DIR}/../common/common-base.sh" ]; then
  # shellcheck disable=SC1091
  source "${SCRIPT_DIR}/../common/common-base.sh"
fi

if ! declare -F infrazero_load_provider_adapter >/dev/null 2>&1; then
  echo "[db] common-base.sh missing infrazero_load_provider_adapter; cannot continue" >&2
  exit 1
fi
infrazero_load_provider_adapter "$SCRIPT_DIR"

if [ -f "${SCRIPT_DIR}/db-patroni.sh" ]; then
  # shellcheck disable=SC1091
  source "${SCRIPT_DIR}/db-patroni.sh"
elif [ -f "${SCRIPT_DIR}/../common/db-patroni.sh" ]; then
  # shellcheck disable=SC1091
  source "${SCRIPT_DIR}/../common/db-patroni.sh"
else
  echo "[db] missing shared db-patroni.sh" >&2
  exit 1
fi

infrazero_load_env_files \
  /etc/infrazero/db.env \
...
apt_get() {
  ...
  INFRAZERO_APT_CLEAN_LISTS="${INFRAZERO_APT_CLEAN_LISTS:-false}" \
    infrazero_apt_get "db" "$@"
}

infrazero_require_env "DB_TYPE" "db"
infrazero_require_env "DB_VERSION" "db"
infrazero_require_env "DATABASES_JSON" "db"
infrazero_require_env "S3_ACCESS_KEY_ID" "db"
infrazero_require_env "S3_SECRET_ACCESS_KEY" "db"
infrazero_require_env "S3_ENDPOINT" "db"
infrazero_require_env "S3_REGION" "db"
infrazero_require_env "DB_BACKUP_BUCKET" "db"
infrazero_require_env "K3S_NODE_CIDRS" "db"
...
if ! command -v aws >/dev/null 2>&1; then
  if declare -F infrazero_ensure_aws_cli >/dev/null 2>&1; then
    infrazero_ensure_aws_cli "db"
  else
    echo "[db] common-base.sh missing infrazero_ensure_aws_cli" >&2
  fi
fi
...
find_db_volume_device() {
  # Volume discovery is provider-specific; see the provider adapter.
  provider_find_data_volume "$@"
}

wait_for_db_volume_device() {
  local volume_name="${1:-}"
  local default_attempts default_sleep
  read -r default_attempts default_sleep <<< "$(provider_volume_wait_defaults)"
  local attempts="${DB_VOLUME_ATTACH_WAIT_ATTEMPTS:-$default_attempts}"
  local sleep_seconds="${DB_VOLUME_ATTACH_WAIT_SECONDS:-$default_sleep}"
  local attempt
  local found=""
...
find_volume_device() {
  local device=""
  device=$(provider_find_data_volume "${VOLUME_NAME:-}" || true)
  if [ -z "$device" ]; then
...
setup_replication_primary() {
  ...
  # Statically-safe default for the Patroni DCS: it does not depend on
  # replica_count, so adding replicas never triggers a pending_restart loop.
  # Override with DB_MAX_WAL_SENDERS if more senders are needed.
...
# postgresql.conf (e.g. max_wal_senders vs DCS max_wal_senders) and can cause
# pending_restart loops on fresh deploys.
...
setup_patroni() {
  ...
  infrazero_require_env "PATRONI_SCOPE" "db"
  infrazero_require_env "PATRONI_ETCD_HOSTS" "db"
...
PY
  ...
  # Build etcd3 hosts list: "host1:2391,host2:2391" into a YAML list
...
"
  ...
  local patroni_pg_hba
  patroni_pg_hba="$(render_patroni_pg_hba "$repl_user" "${PRIVATE_CIDR:-}")"
...
PATRONI_EOF
  ...
  # On redeploys the cluster already exists in etcd; Patroni will not recreate users -
  # only the initial bootstrap creates them. Sync both passwords so replicas can authenticate.
  ...
  # Reads hosts from patroni.yml so it works with any etcd topology.
