...
infrazero_require_env "KUBERNETES_FQDN" "infisical-admin-secret"
infrazero_require_env "INFISICAL_FQDN" "infisical-admin-secret"
infrazero_require_env "INFISICAL_SITE_URL" "infisical-admin-secret"
infrazero_require_env "INFISICAL_ORGANIZATION" "infisical-admin-secret"
infrazero_require_env "INFISICAL_PROJECT_NAME" "infisical-admin-secret"
infrazero_require_env "S3_ACCESS_KEY_ID" "infisical-admin-secret"
infrazero_require_env "S3_SECRET_ACCESS_KEY" "infisical-admin-secret"
infrazero_require_env "S3_ENDPOINT" "infisical-admin-secret"
infrazero_require_env "S3_REGION" "infisical-admin-secret"
infrazero_require_env "DB_BACKUP_BUCKET" "infisical-admin-secret"
infrazero_require_env "INFISICAL_DB_BACKUP_AGE_PRIVATE_KEY" "infisical-admin-secret"
...
apt_get() {
  ...
  INFRAZERO_APT_CLEAN_LISTS="${INFRAZERO_APT_CLEAN_LISTS:-false}" \
    infrazero_apt_get "infisical-admin-secret" "$@"
}
...
if ! command -v aws >/dev/null 2>&1; then
  if declare -F infrazero_ensure_aws_cli >/dev/null 2>&1; then
    infrazero_ensure_aws_cli "infisical-admin-secret"
  else
    echo "[infisical-admin-secret] common-base.sh missing infrazero_ensure_aws_cli" >&2
  fi
fi
...
ensure_gitops_repo() {
  ...
  infrazero_require_env "ARGOCD_APP_REPO_URL" "infisical-admin-secret"
  infrazero_require_env "GH_TOKEN" "infisical-admin-secret"
  local auth_header
