...
emit_postcheck() {
  local component="$1"
  local status="${2:-ok}"
  local marker="INFRAZERO_POSTCHECK role=node2 component=${component} status=${status}"
  echo "[k3s-agent] ${marker}"
...
k3s_agent_ready="false"
for i in {1..60}; do
  if systemctl is-active --quiet k3s-agent; then
    k3s_agent_ready="true"
    break
  fi
  sleep 2
done
if [ "$k3s_agent_ready" != "true" ]; then
  echo "[k3s-agent] k3s-agent did not become active in time" >&2
  exit 1
fi
emit_postcheck "k3s-agent" "ok"
