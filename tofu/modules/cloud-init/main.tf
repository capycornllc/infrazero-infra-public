# Shared cloud-init rendering for all providers.
# Providers call this module and pass provider deltas via route_mode,
# extra_bastion_env / extra_egress_env and include_pgbouncer_env_in_db_role.
# The canonical cloud-init template (incl. bootstrap error handling and the
# route_mode-gated default-route-repair bootcmd) lives in templates/.
...
locals {
  ...
  cloud_init_template_bastion = templatefile(local.cloud_init_template_path, {
    ...
    bastion_env = concat(local.bastion_env_lines, var.extra_bastion_env, [
  ...
  cloud_init_template_egress = templatefile(local.cloud_init_template_path, {
    ...
    admin_users_json_b64                = var.admin_users_json_b64
    debug_root_password                 = local.debug_root_password_escaped
    egress_env                          = concat(local.egress_env_lines, var.extra_egress_env)
  ...
  cloud_init_template_db = templatefile(local.cloud_init_template_path, {
    ...
    bastion_env                         = []
    node_env                            = []
    node_role_env                       = concat(local.db_env_lines, var.include_pgbouncer_env_in_db_role ? local.pgbouncer_env_lines : [])
  })
  ...
  cloud_init_rendered_db_replica = length(var.db_replicas) > 0 ? templatefile(local.cloud_init_template_path, {
    ...
    bootstrap_sha256                    = try(var.bootstrap_artifacts["db-replica"].sha256, "")
    db_volume_name                      = var.db_volume.name
    db_volume_format                    = var.db_volume.format
    private_cidr                        = var.private_cidr
    bastion_private_ip                  = var.servers.bastion.private_ip
    ...
    bastion_env                         = []
    node_env                            = []
    node_role_env = concat(local.db_replica_env_lines, local.db_env_lines, [
