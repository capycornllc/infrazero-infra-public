...
module "cloud_init" {
  ...
  # On OVH, bastion has direct internet via Floating IP - no egress policy
  # routing needed. EGRESS_PRIVATE_IP is intentionally omitted so
  # infrazero-egress-routing.sh skips.
  extra_bastion_env = []
  extra_egress_env = []
  include_pgbouncer_env_in_db_role = true
}
