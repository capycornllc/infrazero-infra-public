...
if [ -z "${S3_ENDPOINT:-}" ] || [ -z "${INFRA_STATE_BUCKET:-}" ] || [ -z "${BOOTSTRAP_PRESIGN_EXPIRY:-}" ]; then
  echo "[offload-egress-secrets] required env missing (S3_ENDPOINT, INFRA_STATE_BUCKET, BOOTSTRAP_PRESIGN_EXPIRY)" >&2
  exit 1
fi
...
PAYLOAD_URL=$(aws --endpoint-url "$S3_ENDPOINT" s3 presign "s3://${INFRA_STATE_BUCKET}/${OBJECT_KEY}" --expires-in "$BOOTSTRAP_PRESIGN_EXPIRY")
PAYLOAD_SHA256=$(sha256sum "$PAYLOAD_FILE" | awk '{print $1}')

"$PYTHON_BIN" - "$TFVARS_PATH" "$PAYLOAD_URL" "$PAYLOAD_SHA256" <<'PY'
import json
import pathlib
import sys
