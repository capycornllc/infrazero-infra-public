...
| `provider_admin_useradd_options <username>` | Печатает дополнительные опции `useradd` для локальных platform-admin пользователей. По умолчанию отсутствует и ничего не меняет. OVH использует `-N`, потому что образ может уже содержать группу `admin`. |
